...
class InstallTest(fake_filesystem_unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.mock_run_command_patcher = unittest.mock.patch('install._run_command')
  ...
  def test_find_extensions_dir_for_extension(self):
    ...
    ext_dir = install.find_extensions_dir_for_extension('sample_1',
                                                        extensions_dirs)
    ...
    extensions_dirs = install.get_extensions_dirs(self.project_root)
    ext_dir = install.find_extensions_dir_for_extension('internal_ext',
                                                        extensions_dirs)
    ...
    extensions_dirs = install.get_extensions_dirs(
        self.project_root, extra_extensions_dirs=[self.testing_extensions_dir])
    ext_dir = install.find_extensions_dir_for_extension('test_sample',
                                                        extensions_dirs)
  ...
  def test_add_test_extension_without_flag_fails(self):
    """Tests add command with a test extension."""
    with unittest.mock.patch('sys.argv', ['install.py', 'add', 'test_sample']):
      with self.assertRaises(SystemExit):
        install.main()

  def test_add_invalid_extension(self):
    """Tests add command with an invalid extension."""
    with unittest.mock.patch('sys.argv', ['install.py', 'add', 'nonexistent']):
      with unittest.mock.patch(
  ...
  def test_update_extension(self):
    ...
    with unittest.mock.patch('sys.argv', ['install.py', 'update', 'sample_1']):
      install.main()
    self.mock_run_command.assert_called_once_with(
  ...
  def test_remove_extension(self):
    ...
    with unittest.mock.patch('sys.argv', ['install.py', 'remove', 'sample-1']):
      install.main()
    self.mock_run_command.assert_called_once_with(
  ...
  def test_enable_extension(self):
    ...
    with unittest.mock.patch('sys.argv', ['install.py', 'enable', 'sample_1']):
      install.main()
    self.mock_run_command.assert_called_once_with(
  ...
  def test_disable_extension(self):
    ...
    with unittest.mock.patch('sys.argv', ['install.py', 'disable', 'sample_1']):
      install.main()
    self.mock_run_command.assert_called_once_with(
...
✓ both-enabled (3.0.0)
 ...
 Enabled (Workspace): true
    ...
    with unittest.mock.patch(
        'sys.stdout', new_callable=io.StringIO) as expected_stdout:
  ...
  def test_list_extensions_no_installed(self):
    ...
    expected_extensions = {
        'sample_1': install.ExtensionInfo(name='sample_1', available='1.0.0'),
    }
    with unittest.mock.patch(
        'sys.stdout', new_callable=io.StringIO) as expected_stdout:
      install._print_extensions_table(expected_extensions)
  ...
  def test_find_extensions_dir_for_nonexistent_extension(self):
    ...
    extensions_dirs = install.get_extensions_dirs(self.project_root)
    ext_dir = install.find_extensions_dir_for_extension('nonexistent',
                                                        extensions_dirs)
  ...
  def test_check_gemini_version_raises_error(self):
    ...
    self.assertIn("Could not determine Gemini CLI version", str(cm.exception))
