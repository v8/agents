  ...
  for line in output.splitlines():
    ...
    if not is_indented:
      ...
      if (stripped_line.startswith('Source:') and
          '(Type: link)' in stripped_line):
        current_ext.linked = True
      elif (stripped_line.startswith('Enabled (Workspace):') and
            'true' in stripped_line):
        current_ext.enabled_for_workspace = True
      elif (stripped_line.startswith('Enabled (User):') and
            'true' in stripped_line):
...
def _print_extensions_table(data: dict[str, ExtensionInfo]) -> None:
  ...
  for name, ext_data in data.items():
    ...
    col_widths['USER'] = max(col_widths['USER'],
...
def get_extensions_from_dir(extensions_dir: Path) -> list[str]:
  ...
  if not extensions_dir.exists():
    return []
  return [p.parent.name for p in extensions_dir.glob('*/gemini-extension.json')]
  ...
  for extension in extensions:
    if command == 'add':
      source_dirs = get_extensions_dirs(
          project_root, extra_extensions_dirs=extra_extensions_dirs)
      source_dir = find_extensions_dir_for_extension(extension, source_dirs)
      if not source_dir:
