...
- In Java, methods annotated with `@CalledByNative` are called by C++.
  - In C++, these appear as methods with a "Java\_" prefix.
- In Java, methods within interfaces annotated with `@NativeMethods` are calls
  into C++.
  - In C++, these appear as methods with a "JNI\_" prefix.
  - If the first parameter is of type `long` and has a prefix of "native", then
    it maps to a method of the same name on the C++ class that comes after the
    "native" prefix. Eg. `void foo(long nativeBarImpl)` means you need to find
    `BarImpl::Foo` in C++.
