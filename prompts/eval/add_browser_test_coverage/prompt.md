...
2. Check that the device's `opened` attribute is `true`
3. Close the mock device with `close()`
4. Check that the device's `opened` attribute is now `false`
5. Check that the device is still in the array returned by
