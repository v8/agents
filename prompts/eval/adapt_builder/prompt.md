...
Create a new builder named `win-blink-asan-rel` that builds and runs web tests
on Windows with address sanitization enabled. The builder should be closely
modeled on `linux-blink-asan-rel`.
...
- Once the `.star` changes have been written, remember to run
  `lucicfg generate main.star` from `//infra/config/` to generate files for LUCI
  to consume.
